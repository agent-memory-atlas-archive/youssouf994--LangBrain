...
Tutti gli endpoint disponibili in FastAPI ([`app/api/main.py`](file:///home/<utente>/LangBrain/app/api/main.py)):
...
Gli interrupt HITL possono essere collocati **ovunque nel flusso del grafo** tramite il wrapper universale in [`app/graph/builder.py`](file:///home/<utente>/LangBrain/app/graph/builder.py) e gestiti dinamicamente via API senza riavviare il sistema.
...
> [!NOTE]
> Il System Prompt di Arbitrato Semantico usato dal MAO è definito in [`app/graph/orchestrator.py`](file:///home/<utente>/LangBrain/app/graph/orchestrator.py) come `_OVERRIDE_SYSTEM_PROMPT`.
